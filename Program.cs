...
namespace _2222_____
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            ParameterizedThreadStart[] paramThread = new ParameterizedThreadStart[Convert.ToInt32(NUM_OF_THREADS)];
            Thread[] thread = new Thread[Convert.ToInt32(NUM_OF_THREADS)];
            for (int i = 0; i < Convert.ToInt32(NUM_OF_THREADS); i++)
            {
                paramThread[i] = new ParameterizedThreadStart(Fun);
                thread[i] = new Thread(paramThread[i]);
                thread[i].Start(i);
            }
        }
    }
